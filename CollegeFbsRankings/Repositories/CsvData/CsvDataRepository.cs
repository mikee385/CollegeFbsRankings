...
using CollegeFbsRankings.Repositories.Memory;
using CollegeFbsRankings.Seasons;
using CollegeFbsRankings.Teams;

namespace CollegeFbsRankings.Repositories.CsvData
{
    public class CsvDataRepository : ICollegeFbsRepository
    {
        ...
        public void AddCsvData(int year, int numWeeksInRegularSeason, TextReader fbsTeamCsvData, TextReader fbsGameCsvData)
        {
            ...
            ReadGameData(season, fbsGameCsvData);

            var currentWeek = NumCompletedWeeksInSeason(season.ID);
        ...
        private void ReadGameData(Season season, TextReader reader)
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (Char.IsDigit(line[0]))
                {
                    ...
                    var seasonType = (week > season.NumWeeksInRegularSeason) ? eSeasonType.PostSeason : eSeasonType.RegularSeason;
        ...
        public int NumCompletedWeeksInSeason(SeasonID season)
        {
            return _games.Where(game => game.Season.ID == season).Completed().RegularSeason().Max(game => game.Week);
        }
    }
}
